...
// PassiveFailureReporter is implemented by the health checker to receive
// proxy-detected upstream failures (Phase G passive failure integration).
type PassiveFailureReporter interface {
	ReportPassiveFailure(serviceName string, err error)
}
...
type Handler struct {
	...
	// passiveReporter is optional; nil means passive failure reporting is disabled.
	passiveReporter PassiveFailureReporter
...
// SetPassiveReporter registers the health checker for passive failure reporting.
// Must be called before the server starts accepting traffic.
func (h *Handler) SetPassiveReporter(r PassiveFailureReporter) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.passiveReporter = r
}
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// ── Request ID (Phase F) ──────────────────────────────
	// Middleware already ran if wired through server; this is a fallback
	// so the proxy works standalone in tests too.
	...
	if !h.sem.TryAcquire() {
		h.metrics.RecordRequest("", 0, true)
	...
	if buildErr != nil {
		h.log.Error("upstream_request_build_failed",
			logger.F("request_id", reqID),
			logger.F("error", buildErr.Error()))
		h.metrics.RecordRequest(svc, 0, true)
	...
	if doErr != nil {
		...
		h.log.Error("upstream_error",
			logger.F("request_id", reqID),
			logger.F("service", svc),
			logger.F("upstream", upstream),
			logger.F("error", sanitizeUpstreamError(doErr)))
		h.metrics.RecordRequest(svc, latencyMs, true)

		// Passive failure reporting (Phase G).
		h.mu.Lock()
		reporter := h.passiveReporter
		h.mu.Unlock()
		if reporter != nil {
			reporter.ReportPassiveFailure(svc, doErr)
		}
	...
	h.metrics.RecordRequest(svc, latencyMs, isError)
	...
	h.log.Info("request",
		...
		logger.F("path", r.URL.Path),
		logger.F("service", svc),
		logger.F("status", fmt.Sprintf("%d", resp.StatusCode)),
		logger.F("latency_ms", fmt.Sprintf("%d", latencyMs)),
	)

	// ── Forward response with limits ─────────────────────
	if err := writeResponse(w, resp, lim); err != nil {
		h.log.Error("response_write_error",
			logger.F("request_id", reqID),
			logger.F("error", err.Error()))
...
// Metrics returns the metrics recorder for this handler (for API access).
